...
def headerdb(request):
    ...
    context_ins={'loginas':loginas,'header_dept_list':dept_list,'dept_list':dept_list,
		 ...
		 'exp_job_list':exp_job_list,
   'fname':fname,'clg_id':clg_id,'register_success':register_success,'cannot_contribute':cannot_contribute,'forgotpass_emailsent':forgotpass_emailsent,'contribute_success':contribute_success }
...
def fill_experience(request):
	...
	print "hii",other_comments
	
        if package == "":
            package = 0
        elif package.isdigit():
            package = float(package) #float(package)*float(request.POST.get('amount'))
        ...
        # IF its a new company then add it to the list
        if new_company != "":
